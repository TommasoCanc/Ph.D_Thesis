...
file.number <- which(osf_ls_files(osf_files[2, ], pattern = "xlsx")[ ,1] == 
                       "European_Odonata_checklist.xlsx")
osf_download(osf_ls_files(osf_files[2, ], pattern = "xlsx")[file.number, ], 
             path = main.path)
rm(file.number)
...
file.number <- which(osf_ls_files(osf_files[2, ], pattern = "csv")[ ,1] == 
                       "European_Odonata_Taxonomy.csv")
osf_download(osf_ls_files(osf_files[2, ], pattern = "csv")[file.number, ], 
             path = main.path)
rm(file.number)
