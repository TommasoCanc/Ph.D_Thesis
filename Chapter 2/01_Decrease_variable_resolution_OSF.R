...
file.number <- which(osf_ls_files(osf_files[2, ])[ ,1] == "Present.zip")
osf_download(osf_ls_files(osf_files[2, ])[file.number, ], path = main.path)
rm(file.number)
...
file.number <- which(osf_ls_files(osf_files[2, ])[ ,1] == "wc2.1_5m_elev.tif")
osf_download(osf_ls_files(osf_files[2, ])[file.number, ], path = main.path) 
rm(file.number)
...
file.number <- which(osf_ls_files(osf_files[2, ])[ ,1] == "World_Countries.zip")
osf_download(osf_ls_files(osf_files[2, ])[file.number, ], 
...
# Save rasters
for(i in 1:12){
  writeRaster(t_min.sub[[i]], filename = paste0("./Present/tMin_crop/", 
              names(t_min.sub)[i], ".tif"), format = "GTiff", overwrite = FALSE)
}; rm(i)

for(i in 1:12){
  writeRaster(t_max.sub[[i]], filename = paste0("./Present/tMax_crop/", 
              names(t_max.sub)[i], ".tif"), format = "GTiff", overwrite = FALSE)
}; rm(i)

for(i in 1:12){
  writeRaster(prec.sub[[i]], filename = paste0("./Present/Prec_crop/", 
              names(prec.sub)[i], ".tif"), format = "GTiff", overwrite = FALSE)
}; rm(i)

for(i in 1:19){
  writeRaster(bioVar[[i]], filename = paste0("./Present/bioVar_crop/", 
              names(bioVar)[i], ".tif"), format = "GTiff", overwrite = FALSE)
}; rm(i)

writeRaster(elev.5.sub, filename = "./elevation_5_crop.tif", 
...
file.number <- which(osf_ls_files(osf_files[2, ])[ ,1] == "MIROC_2050.zip")
osf_download(osf_ls_files(osf_files[2, ])[file.number, ], 
             path = main.path); rm(file.number)
...
# Save rasters
for(i in 1:12){
  writeRaster(t_min.sub[[i]], filename = paste0("./MIROC_2050/tMin_crop/", 
              names(t_min.sub)[i], ".tif"), 
              format = "GTiff", overwriteFALSE= FALSE)}; rm(i)

for(i in 1:12){
  writeRaster(t_max.sub[[i]], filename = paste0("./MIROC_2050/tMax_crop/", 
              names(t_max.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:12){
  writeRaster(prec.sub[[i]], filename = paste0("./MIROC_2050/Prec_crop/", 
              names(prec.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:19){
  writeRaster(bioVar[[i]], filename = paste0("./MIROC_2050/bioVar_crop/", 
              names(bioVar)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)
...
file.number <- which(osf_ls_files(osf_files[2, ])[ ,1] == "MIROC_2070.zip")
osf_download(osf_ls_files(osf_files[2, ])[file.number, ], 
             path = main.path); rm(file.number)
...
# Save rasters
for(i in 1:12){
  writeRaster(t_min.sub[[i]], filename = paste0("./MIROC_2070/tMin_crop/", 
              names(t_min.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:12){
  writeRaster(t_max.sub[[i]], filename = paste0("./MIROC_2070/tMax_crop/", 
              names(t_max.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:12){
  writeRaster(prec.sub[[i]], filename=paste0("./MIROC_2070/Prec_crop/", 
              names(prec.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:19){
  writeRaster(bioVar[[i]], filename=paste0("./MIROC_2070/bioVar_crop/", 
              names(bioVar)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)
...
file.number <- which(osf_ls_files(osf_files[2, ])[ ,1] == "NOR_2050.zip")
osf_download(osf_ls_files(osf_files[2, ])[file.number, ], 
             path = main.path); rm(file.number)
...
# Save rasters
for(i in 1:12){
  writeRaster(t_min.sub[[i]], filename = paste0("./NOR_2050/tMin_crop/", 
              names(t_min.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:12){
  writeRaster(t_max.sub[[i]], filename = paste0("./NOR_2050/tMax_crop/", 
              names(t_max.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:12){
  writeRaster(prec.sub[[i]], filename = paste0("./NOR_2050/Prec_crop/", 
              names(prec.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:19){
  writeRaster(bioVar[[i]], filename = paste0("./NOR_2050/bioVar_crop/", 
              names(bioVar)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)
...
file.number <- which(osf_ls_files(osf_files[2, ])[ ,1] == "NOR_2070.zip")
osf_download(osf_ls_files(osf_files[2, ])[file.number, ], 
             path = main.path); rm(file.number)
...
# Save rasters
for(i in 1:12){
  writeRaster(t_min.sub[[i]], filename = paste0("./NOR_2070/tMin_crop/", 
              names(t_min.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:12){
  writeRaster(t_max.sub[[i]], filename = paste0("./NOR_2070/tMax_crop/", 
              names(t_max.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:12){
  writeRaster(prec.sub[[i]], filename = paste0("./NOR_2070/Prec_crop/", 
              names(prec.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:19){
  writeRaster(bioVar[[i]], filename = paste0("./NOR_2070/bioVar_crop/",
              names(bioVar)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)
...
#----------#
# Download climate variables MIROC 2050
file.number <- which(osf_ls_files(osf_files[2, ])[ ,1] == "BCC_2050.zip")
osf_download(osf_ls_files(osf_files[2, ])[file.number, ], 
             path = main.path); rm(file.number)
...
# Save rasters
for(i in 1:12){
  writeRaster(t_min.sub[[i]], filename = paste0("./BCC_2050/tMin_crop/", 
              names(t_min.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:12){
  writeRaster(t_max.sub[[i]], filename = paste0("./BCC_2050/tMax_crop/", 
              names(t_max.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:12){
  writeRaster(prec.sub[[i]], filename = paste0("./BCC_2050/Prec_crop/", 
              names(prec.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:19){
  writeRaster(bioVar[[i]], filename = paste0("./BCC_2050/bioVar_crop/", 
              names(bioVar)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)
...
file.number <- which(osf_ls_files(osf_files[2, ])[ ,1] == "BCC_2070.zip")
osf_download(osf_ls_files(osf_files[2, ])[file.number, ], 
             path = main.path); rm(file.number)
...
# Save rasters
for(i in 1:12){
  writeRaster(t_min.sub[[i]], filename = paste0("./BCC_2070/tMin_crop/", 
              names(t_min.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:12){
  writeRaster(t_max.sub[[i]], filename = paste0("./BCC_2070/tMax_crop/", 
              names(t_max.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:12){
  writeRaster(prec.sub[[i]], filename = paste0("./BCC_2070/Prec_crop/", 
              names(prec.sub)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)

for(i in 1:19){
  writeRaster(bioVar[[i]], filename = paste0("./BCC_2070/bioVar_crop/", 
              names(bioVar)[i], ".tif"), 
              format = "GTiff", overwrite = FALSE)}; rm(i)